...
class IlHospitalLicensesSpider(ExcelFeedSpider,DataFormatterMixin,LookupDatareaderMixin):
    name = '1390_il_home_service_agency_licenses'
    ...
    custom_settings = {
        'FILE_NAME':Utils.getRundateFileName('AI-1390_Licenses_Home_Service_Agency_IL_CurationReady'),
        'JIRA_ID':'AI_1390',
        'DOWNLOAD_DELAY':5,
    ...
    def parse_row(self, response, row):
        ...
        il.add_value('sourceName', 'IL_Home_Service_Agency_Licenses')
        il.add_value('permit_type', "business_license")
        name=self._getDBA(row['Home Services Agencies'])
        company_name=str(name[0]).replace(' -','') if ' -' in str(name[0]) else name[0]
        address=self.format__address_4(row['Address'],row['City'],'IL',row['Zip'])
